...
- **On-Chain Security:** User top-ups are standard on-chain transactions (ETH, USDC, or MINIDEV on Base) sent directly to your developer wallet — 0% platform fee, you keep everything.
- **Reliable Billing:** Our `track`/`capture`/`release` flow ensures users are only billed for successful AI operations.
- **Auto Buyback & Burn:** Optionally route a portion of revenue into an automated buyback-and-burn pipeline for your token — fully on-chain, Turnkey-managed, no manual steps.
...
2. **SDK**: Review the [SDK documentation](./sdk-reference) and API endpoints
